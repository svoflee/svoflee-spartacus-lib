...
package com.svoflee.spartacus.lib.dal.nosql.redis.elector;
 ...
 * 与每次使用setNx争夺的分布式锁不同，Master用setNX争夺Master Key成功后，
 * 会不断的更新key的expireTime，保持自己的master地位，直到自己倒掉了不能再更新为止。
...
public class MasterElector implements Runnable {
    ...
    private static Logger logger = Logger.getLogger(MasterElector.class);
    ...
    public MasterElector(JedisPool jedisPool, int intervalSecs) {
        ...
        expireSecs = intervalSecs + (intervalSecs / 2);
    }
